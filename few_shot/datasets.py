...
#         """
#         if subset not in ('images_background', 'images_evaluation'):
...
#         self.unique_characters = sorted(self.df['id'].unique())
#         self.class_name_to_id = {self.unique_characters[i]: i for i in range(self.num_classes())}
#         self.df = self.df.assign(class_id=self.df['id'].apply(lambda c: self.class_name_to_id[c]))
...
class MiniImageNet(Dataset):
    ...
    def index_subset(subset):
        ...
        # Quick first pass to find total for tqdm bar
        subset_len = 0
        for root, folders, files in os.walk(DATA_PATH + '/images/'.format(subset)):
            subset_len += len([f for f in files if f.endswith('.png')])
